...
<p style='margin-bottom: 0; text-align: justify;'> La question 'Qui veut peut?' nous amène à réfléchir sur les ingrédients de la réussite individuelle. Cet adage suggère que la volonté et l'effort personnel sont les moteurs clés de l'épanouissement, mettant en avant l'importance de l'autodétermination. </p> 
<p style='margin-bottom: 0; text-align: justify;'>Cependant, une analyse critique nous pousse à considérer l'impact des variables socio-démographiques et environnementales.</p>
<p style='text-align: justify;'>
    <strong> Est-ce que la réussite dépend uniquement de l'effort individuel, ou est-elle également influencée par le contexte socio-économique et culturel ? </strong></p>
<p style='text-align: justify;'> Pour examiner cette question, nous ferons une étude des notes de mathématiques de 395 lycéens issus de deux lycées portugais, à savoir Gabriel Pereira (GP) et Mousinho da Silveira(MS), basé sur 33 variables scolaire et socio-démographiques.  Nous vous invitons à découvrir ces éléments dans les items suivants :</p>
<p style='text-align: justify;'>
 <ol>
 <li> Le premier vous permettra de découvrir la base de données. Vous pourrez, en fonction de vos préférences, choisir le nombre de lignes à afficher (max les 100 premières).</li>
 <li> Le second item vous permettra de mieux connaître les variables. En effet, vous pourrez sélectionner le(s) variable(s) que vous souhaitez et une description de celle(s)-ci apparaîtra.</li>
      ...
      tabItem(
        ...
        fluidRow(
          column(
            ...
            HTML("<div class='justify-text'>
 <p style='text-align: justify;'>Dans cette étude, les données ont été collectées auprès de deux écoles, Gabriel Pereira (GP) et Mousinho da Silveira (MS). Il faut garder à l'esprit lors de la comparaison que seuls 46 étudiants sur 395 sont allés à Mousinho da Silveira. Il faut donc être très vigilant quand à l'interprétation et aux conclusions qu'on pourrait en tirer, pour cela nous ferons les comparaisons uniquement en proportion.</p>
 </div>"),
      ...
      tabItem(tabName="ml",
              fluidRow(
                column(
                  ...
                  HTML("<div class='justify-text'>
 <p style='text-align: justify;'> Dans la section précédente, nous avons initialement exploré les deux lycées. Cependant, en raison d'un manque de données, nous avons décidé qu'il serait plus judicieux de mener une analyse sur l'ensemble des données disponibles. Face à la présence de 32 variables explicatives potentielles dans notre base de données, nous avons opté pour l'utilisation du machine learning afin d'identifier les variables ayant une influence réelle sur les résultats de G3. </p>
 </div>"),
                  HTML("<h2><strong><span style='color: #665191;'> Matrice de corrélation </span></strong></h2>"),
                  HTML("<div class='justify-text'>
 <p style='text-align: justify;'> Pour commencer, nous avons voulu afficher une matrice de corrélation. Il faut savoir qu'une matrice de corrélation se fait généralement avec des données numérique, ce qui n'est pas le cas de toutes nos variables. Pour faire face à ce problème nous avons créer data2 : </p>
 </div>"),
 ...
 <p>Enfin, le dernier graphique explore la relation entre les notes du premier trimestre (T1) et le niveau d'éducation de la mère. On constate une corrélation positive, suggérant que les étudiants dont les mères ont un niveau d'éducation plus élevé ont tendance à obtenir de meilleures notes. Cette association pourrait être attribuée à un environnement familial favorisant l'éducation et l'apprentissage.</p>")
      ...
      tabItem(
        ...
        tags$head(
          tags$style(HTML("
            .prediction-box {
              border: 1px solid #ddd; /* Bordure du cadre */
        ...
        fluidRow(
          column(
            ...
            HTML("<h4><span style='color:#665191;'> Nous voila enfin arrivé à l'étape la plus attendue : la prédiction ! </span><h4>"),
            HTML("<p style='font-size: 14px;'>Pour estimer la note finale, ajustez les paramètres ci-dessous en fonction du profil académique et des performances passées.</p>"),
            sidebarLayout(
              sidebarPanel(
              ...
              mainPanel(
                div(class = "prediction-box",
                    HTML("<h4><span style='color:#a05195;'><strong>Note prédite: </strong></span>"),
                    textOutput("valeur_affichee", inline = TRUE)
                )
